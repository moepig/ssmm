...
`cli` は非対話の場合に最終結果を待つ。対話の場合はスナップショットを `tui` に渡し、選択または更新要求を受け取る。検索世代の終了と次の世代の開始も `cli` が制御する。
...
`session` と `openssh` は実行計画を生成する。実際の起動と終了処理は `process` が行う。標準入出力は子プロセスへ渡し、proxy の通信データを ssmm が行単位で解析する処理はない。
