...
プロファイルの選択順序と環境変数の扱いは、[設定の ssmm プロファイルと AWS プロファイル](configuration.md#ssmm-プロファイルと-aws-プロファイル) を参照。
...
以下では、既存の AWS プロファイル `company-prod` と、接続先を含むリージョン `ap-northeast-1` を使う。`init` や ssmm プロファイルの作成は不要である。

1. `ssmm list -p company-prod -r ap-northeast-1` で EC2 一覧を取得する。
2. `ssmm connect -p company-prod -r ap-northeast-1` で選択画面を開く。
...
| 症状 | 確認点 |
| --- | --- |
| AWS プロファイルが見つからない | ssmm 設定の `aws_profile` または `AWS_PROFILE` が指す名前が、AWS の共有設定・認証情報ファイルに存在するか |
