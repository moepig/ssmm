...
TARGET を指定した場合は、全検索範囲の EC2 取得に成功し、候補が 1 件で running なら選択画面を開かずに接続する。0 件または複数件ならエラーになる。TARGET を省略した場合は、操作端末で接続先を選択する。操作端末がない場合も、検索が完了して一意な running の候補がある場合だけ接続する。
